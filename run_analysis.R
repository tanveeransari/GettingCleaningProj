...
#4. is done here before the other steps - label the dataset with descriptive variable names
...
# The act_id and subj_id columns were what we just added from lookup tables
goodCols<-grepl("[Mm]ean",names(master)) | grepl("std",names(master)) |
  grepl("subj_id",names(master))|grepl("act_id",names(master))
